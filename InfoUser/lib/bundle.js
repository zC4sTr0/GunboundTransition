...
var _tank = __webpack_require__(7);
...
var canTerrain = document.getElementById('terrain-canvas');
var ctx = can.getContext('2d');
var ctxTerrain = canTerrain.getContext('2d');
var canText = document.getElementById('text-canvas');
...
var Game = function () {
  function Game() {
    ...
    this.stage = new _stage2.default(this.ctxTerrain, this.canTerrain);
    this.tank = new _tank2.default('Player 1', 100, -200, ctx, ctxTerrain);
    this.tank2 = new _tank2.default('Player 2', 400, -200, ctx, ctxTerrain);
    this.tank3 = new _tank2.default('Player 3', 770, -200, ctx, ctxTerrain);
    this.tank4 = new _tank2.default('Player 4', 1070, -200, ctx, ctxTerrain);
  ...
  _createClass(Game, [{
    key: 'bindKeyDown',
    value: function bindKeyDown(event) {
    ...
    key: 'bindKeyUp',
    value: function bindKeyUp(event) {
      var keyName = event.key;
    ...
    value: function render() {
      ...
      for (var i = 0; i < window.tanks.length; i++) {
      ...
      for (var _i = 0; _i < this.objects.length; _i++) {
        this.objects[_i].render(this.ctx);
        if (this.objects[_i] && (Math.abs(this.objects[_i].x) > 1800 || Math.abs(this.objects[_i].y) > 1800)) {
    ...
    value: function startTurns() {
      ...
      if (this.turnCounter === 1) window.tanks = window.tanks.sort(function (a, b) {
      ...
      var shotKey = function shotKey(e) {
        e.key;
        if (e.key == '1') _this.currentTank.switchShot1(); //switch shot event listener
        if (e.key == '2') _this.currentTank.switchShot2(); //switch shot event listener
      };

      var shotKeyss = function shotKeyss(e) {
        var keyPress = e.key;
        if (e.key == '3' && _this.currentTank.ssCooldown <= 0) _this.currentTank.switchShotss(); //switch shot event listener
      };

      if (this.roundCounter !== 0 && this.roundCounter % 3 === 0 && this.turnCounter === 1) {
        //change wind every 3 rounds
        this.wind.play();
        this.stage.changeWind();
      }

      document.getElementById('turn-queue').innerHTML = '';
      for (var i = 1; i < window.tanks.length + 1; i++) {
        //print queue
      ...
      if (this.turnCounter % window.tanks.length === 0) {
        //if last turn inc round and reset turns
        this.roundCounter++;
        this.turnCounter = 1;
      } else {
        //otherwise inc turns
      ...
      this.interval = setInterval(function () {
        if (_this.currentTank.turnOver) {
          clearInterval(_this.interval);
          _this.time = _this.TIMEOUT;
          clearTimeout(_this.timeout);
        } else {
          let timer = document.getElementById('main-timer');
          //timer.innerHTML = _this.time - 1;
          let t = _this.time * (2000/20) - 200;
          timer.style.backgroundPosition = `0px -${t}px`;
          _this.currentTank.delay += 10;
          _this.time--;
    ...
    value: function over(tank) {
      ...
      window.UI.ctx.fillText(tank.username + ' Wins!', this.can.width / 2, this.can.height / 2 - 200);
    }
  }]);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.getElementById('vol-container').onclick = function () {
...
var Stage = function () {
  function Stage(ctxTerrain, canTerrain) {
    var _this = this;
    ...
    this.img.onload = function () {
      _this.ctx.drawImage(_this.img, 50, 320);
      _this.data = _this.ctx.getImageData(0, 0, _this.img.width, _this.img.height);
    };
    this.shell = null;
    this.holes = [];
  }
...
var _guntanks = __webpack_require__(0);

var _guntanks2 = _interopRequireDefault(_guntanks);
...
var UI = function () {
  ...
  _createClass(UI, [{
    key: 'startAdjustPower',
    value: function startAdjustPower() {
      if (!this.disabled) {
        var power = document.getElementById('power-fill');
        var width = parseInt(power.style.width.slice(0, -2));
    ...
    key: 'stopAdjustPower',
    value: function stopAdjustPower(tank) {
      var _this = this;
      ...
      if (!this.disabled) {
        ...
        clearInterval(this.interval);
        this.interval = null;
        var bar = document.getElementById('power-fill');
        var fullBar = document.getElementById('power-container');
        var power = 100 * (parseInt(bar.style.width.slice(0, -2)) / parseInt(fullBar.style.width.slice(0, -2)));
        tank.prevPower = power / 100 * 1000;
        setTimeout(function () {
          bar.style.width = '5px';
          _this.game.addObject(tank.fire(power));
        }, 100);
      }
    }
  }]);
...
var Shell = function () {
  function Shell(x, y, xVel, yVel, ctx, ctxTerrain) {
  ...
  _createClass(Shell, [{
    ...
    value: function render(ctx) {
      ...
      if (this.hitTest(this.x, this.y, this.radius * 2, this.radius * 2) === 'tank') {
        ...
        this.x += this.xVel + 0.02 * this.windSpeed * Math.cos(this.windAngle * Math.PI / 180) * this.time; //+ wind factor
        this.y = this.y + this.yVel + 0.5 * this.weight * this.time * this.time - 0.02 * this.windSpeed * Math.sin(this.windAngle * Math.PI / 180) * this.time;
    ...
    value: function hitTest(x, y, width, height) {
      ...
      for (var _i2 = 0; _i2 < array.length; _i2++) {
        ...
        if (this.ctxTerrain.getImageData(array[_i2][0], array[_i2][1], 1, 1).data[0] != 0 || this.ctxTerrain.getImageData(array[_i2][0], array[_i2][1], 1, 1).data[1] != 0 || this.ctxTerrain.getImageData(array[_i2][0], array[_i2][1], 1, 1).data[2] != 0 || this.ctxTerrain.getImageData(array[_i2][0], array[_i2][1], 1, 1).data[3] != 0) {
          return 'terrain';
        }
      }
      return false;
    ...
    value: function isCollidedWith(otherObject) {
      ...
      var x2 = otherObject.x + otherObject.width / 2;
      var y2 = otherObject.y + otherObject.height / 2;

      var r1 = this.radius;
      var r2 = otherObject.radius;
...
var _shell6 = __webpack_require__(3);

var _shell7 = _interopRequireDefault(_shell6);

var _shell8 = __webpack_require__(8);

var _shell9 = _interopRequireDefault(_shell8);

var _shellss = __webpack_require__(9);

var _shellss2 = _interopRequireDefault(_shellss);
...
var Tank = function () {
  function Tank(username, x, y, ctx, ctxTerrain) {
  ...
  _createClass(Tank, [{
    ...
    value: function hitTest(x, y, width, height) {
      ...
      for (var _i = 0; _i < array.length; _i++) {
        // console.log(this.ctx.getImageData(array[i][0],array[i][1],1,1).data);
        if (this.ctxTerrain.getImageData(array[_i][0], array[_i][1], 1, 1).data[0] != 0 || this.ctxTerrain.getImageData(array[_i][0], array[_i][1], 1, 1).data[1] != 0 || this.ctxTerrain.getImageData(array[_i][0], array[_i][1], 1, 1).data[2] != 0 || this.ctxTerrain.getImageData(array[_i][0], array[_i][1], 1, 1).data[3] != 0) {
          return true;
        }
      }
      return false;
    ...
    value: function fire(power) {
      ...
      if (this.face === 'right') {
        if (this.shellSelection === 1) {
          var shell = new _shell7.default(this.x + this.width, this.y - 15, power * this.shellSpeed * Math.cos(this.angle * Math.PI / 180), -power * this.shellSpeed * Math.sin(this.angle * Math.PI / 180), this.ctx, this.ctxTerrain);
          return shell;
        } else if (this.shellSelection === 2) {
          var _shell = new _shell9.default(this.x + this.width, this.y - 15, power * this.shellSpeed * Math.cos(this.angle * Math.PI / 180), -power * this.shellSpeed * Math.sin(this.angle * Math.PI / 180), this.ctx, this.ctxTerrain);
          return _shell;
        } else {
          this.shellSelection = 1; //reset selector after ss shot
          var _shell2 = new _shellss2.default(this.x + this.width + 2, this.y - 15, power * this.shellSpeed * Math.cos(this.angle * Math.PI / 180), -power * this.shellSpeed * Math.sin(this.angle * Math.PI / 180), this.ctx, this.ctxTerrain);
          return _shell2;
        }
      } else if (this.face === 'left') {
        if (this.shellSelection === 1) {
          var _shell3 = new _shell7.default(this.x - 16, this.y - 20, -power * this.shellSpeed * Math.cos(this.angle * Math.PI / 180), -power * this.shellSpeed * Math.sin(this.angle * Math.PI / 180), this.ctx, this.ctxTerrain);
          return _shell3;
        } else if (this.shellSelection === 2) {
          var _shell4 = new _shell9.default(this.x - 16, this.y - 20, -power * this.shellSpeed * Math.cos(this.angle * Math.PI / 180), -power * this.shellSpeed * Math.sin(this.angle * Math.PI / 180), this.ctx, this.ctxTerrain);
          return _shell4;
        } else {
          this.shellSelection = 1; //reset selector after ss shot
          var _shell5 = new _shellss2.default(this.x - 20, this.y - 20, -power * this.shellSpeed * Math.cos(this.angle * Math.PI / 180), -power * this.shellSpeed * Math.sin(this.angle * Math.PI / 180), this.ctx, this.ctxTerrain);
          return _shell5;
        }
      }
    }
    ...
    key: 'changeAngle',
    value: function changeAngle(inc) {
      this.angle += inc;
    }
  }, {
    key: 'switchShot1',
    value: function switchShot1(e) {
      this.shellSelection = 1;
    }
  }, {
    key: 'switchShot2',
    value: function switchShot2(e) {
      this.shellSelection = 2;
    }
  }, {
    key: 'switchShotss',
    value: function switchShotss(e) {
...
var _shell = __webpack_require__(3);
...
var Shell2 = function (_Shell) {
  _inherits(Shell2, _Shell);

  function Shell2(x, y, xVel, yVel, ctx, ctxTerrain) {
    _classCallCheck(this, Shell2);

    var _this = _possibleConstructorReturn(this, (Shell2.__proto__ || Object.getPrototypeOf(Shell2)).call(this));
    ...
    _this.damage = 240;
    _this.hitObject = null;
    _this.explosionRadius = 35;
    _this.color = 'red';
    _this.ctxTerrain = ctxTerrain;

    _this.windAngle = window.stage.windAngle;
    _this.windSpeed = window.stage.windSpeed;
    return _this;
  }
...
var ShellSS = function (_Shell) {
  _inherits(ShellSS, _Shell);

  function ShellSS(x, y, xVel, yVel, ctx, ctxTerrain) {
    _classCallCheck(this, ShellSS);

    var _this = _possibleConstructorReturn(this, (ShellSS.__proto__ || Object.getPrototypeOf(ShellSS)).call(this));
    ...
    _this.damage = 350;
    _this.hitObject = null;
    _this.explosionRadius = 70;
    _this.color = 'rgb(255, 236, 130)';
    _this.ctxTerrain = ctxTerrain;
